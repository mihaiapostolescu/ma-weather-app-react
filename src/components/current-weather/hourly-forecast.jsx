    ...
    hourlyForecastItems.push(
      <div className="forecast-item" key={i}>
        <div className="now flexc">
          <p className="forecast-hour">{hourlyDate.toLocaleString().split(",")[1].split(":")[0] + ":" + hourlyDate.toLocaleString().split(",")[1].split(":")[1]}</p>
          <p className="forecast-temperature-hour">{Math.round(data.hourly[0].temp)}&#176;</p>
          <img
            src={pictureSource}
            className="forecast-weather-hour"
            alt="Icon for weather forecast"
            title={data.hourly[i].weather[0].description}
          />
          <div className="humidity flexr centered">
            <p className="forecast-humididty-hour">{Math.round(data.hourly[i].humidity)}</p>
            <img
